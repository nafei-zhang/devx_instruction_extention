...
- 入口与命令：[src/extension.ts](src/extension.ts)  
- Webview 面板与树视图：[src/webview/fetchPanel.ts](src/webview/fetchPanel.ts)  
- GitHub API 调用：[src/github.ts](src/github.ts)  
- 文件写入与编码：[src/utils/fs.ts](src/utils/fs.ts)
