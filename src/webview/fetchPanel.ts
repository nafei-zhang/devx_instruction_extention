...
export class FetchPanel {
  ...
  private readFallback<T>(key: string): T | undefined {
    const fallback = this.fallbackKey(key);
    const workspaceValue = this.ctx.workspaceState.get<T>(fallback);
    if (workspaceValue !== undefined) return workspaceValue;
    return this.ctx.globalState.get<T>(fallback);
  }
  private async writeFallback(key: string, value: unknown, preferWorkspace: boolean): Promise<void> {
    const fallback = this.fallbackKey(key);
    if (preferWorkspace) {
  ...
  private getConfigWriter() {
    ...
    const isNotRegisteredError = (error: any): boolean => {
    ...
    return {
      get<T>(key: string): T | undefined {
        ...
        return panel.readFallback<T>(key);
      },
      async update(key: string, value: unknown) {
        if (preferWorkspace) {
          try {
            ...
            if (!isNotRegisteredError(error)) throw error;
          }
        }
        try {
